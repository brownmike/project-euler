...
def check_horizontal
  ...
  GRID.each do |row|
    row.each_cons(4) do |set|
      candidate = set.inject(&:*)
      product = candidate > product ? candidate : product
    end
  end
  product
end
...
def check_vertical
  product = 0
  0.upto(19) do |i|
    GRID.collect { |row| row[i]}.each_cons(4) do |set|
      candidate = set.inject(&:*)
      product = candidate > product ? candidate : product
    end
  end
...
def check_diagonal
  ...
  0.upto(16) do |row|
    0.upto(16) do |col|
      set = [GRID[row][col], GRID[row+1][col+1], GRID[row+2][col+2], GRID[row+3][col+3]]
      candidate = set.inject(&:*)
      product = candidate > product ? candidate : product
    end
  end
...
product = 0
product = check_horizontal > product ? check_horizontal : product
product = check_vertical > product ? check_vertical : product
product = check_diagonal > product ? check_diagonal : product
